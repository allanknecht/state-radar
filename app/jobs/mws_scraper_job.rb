...
class MwsScraperJob < ApplicationJob
  queue_as :default

  def perform
    mws = MwsScraperService.new
    %i[locacao venda].each do |cat|
      mws.scrape_category(cat, fetch_details: true) do |r|
        upsert_record!(r)
      end
    end
  end
  ...
  def upsert_record!(r)
    ...
    if missing.any?
      warn "[MwsJob] SKIP: faltando #{missing.join(", ")} (link=#{r[:link].inspect})"
      return
    end
    ...
    rec.save!
    warn "[MwsJob] UPSERT OK site=#{r[:site]} cat=#{rec.categoria} code=#{code}"
    rec
  rescue ActiveRecord::RecordInvalid => e
    warn "[MwsJob] VALIDATION FAIL site=#{r[:site]} code=#{code} -> #{e.record.errors.full_messages.join("; ")}"
  rescue => e
    warn "[MwsJob] UPSERT FAIL site=#{r[:site]} code=#{code} -> #{e.class}: #{e.message}"
  end
end
