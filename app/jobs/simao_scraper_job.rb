...
class SimaoScraperJob < ApplicationJob
  queue_as :default
  ...
  #
  # Normalizações e utilitários (mesmos do Solar, duplicados aqui por simplicidade)
  #
  ...
  def upsert_record!(r)
    ...
    if missing.any?
      warn "[SimaoJob] SKIP: faltando #{missing.join(", ")} (link=#{r[:link].inspect})"
      return
    end
    ...
    rec.save!
    warn "[SimaoJob] UPSERT OK site=#{r[:site]} cat=#{rec.categoria} code=#{code}"
    rec
  rescue ActiveRecord::RecordInvalid => e
    warn "[SimaoJob] VALIDATION FAIL site=#{r[:site]} code=#{code} -> #{e.record.errors.full_messages.join("; ")}"
  rescue => e
    warn "[SimaoJob] UPSERT FAIL site=#{r[:site]} code=#{code} -> #{e.class}: #{e.message}"
  end
end
